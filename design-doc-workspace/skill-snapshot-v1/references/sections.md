...
The order below is a sensible default order for the document itself. None of these
sections is mandatory — each entry explains what the section buys the reader, how to
write it well, and what to ask the user when its substance is missing. Skip any
section whose entry doesn't apply; a skipped section needs no apology and no
placeholder.
