...
- **design-doc** — writes *and* reviews design docs via interactive discovery; trade-offs are mandatory (zero-cons = red flag). A user-supplied or house template **governs** (its sections become required); only without a template are sections suggestions.
